...
final class DecisionService: ObservableObject {
    ...
    func loadDecisions() async {
        ...
        do {
            ...
            errorMessage = "Failed to load decisions. Pull to retry."
        }
    }
    ...
    func fetchOutcomes(for decisionId: UUID) async -> [Outcome] {
        do {
            return try database.fetchOutcomes(for: decisionId)
        } catch {
            return []
        }
    }
